...
func validateTxPoolInternals(pool *TxPool) error {
	...
	for addr, txs := range pool.pending {
		...
		if nonce := pool.pendingState.GetNonce(addr); nonce != last+1 {
			return fmt.Errorf("pending nonce mismatch: have %v, combt %v", nonce, last+1)
		}
	}
	return nil
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.comb))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.comb))
	...
	if nonce != 0 {
		t.Fatalf("Invalid nonce, combt 0, got %d", nonce)
	}
	...
	nonce = pool.State().GetNonce(address)
	if nonce != 2 {
		t.Fatalf("Invalid nonce, combt 2, got %d", nonce)
	}
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	if tx := pool.pending[addr].txs.items[0]; tx.Hash() != tx2.Hash() {
		t.Errorf("transaction mismatch: have %x, combt %x", tx.Hash(), tx2.Hash())
	}
...
func TestTransactionDropping(t *testing.T) {
	...
	if pool.pending[account].Len() != 3 {
		t.Errorf("pending transaction mismatch: have %d, combt %d", pool.pending[account].Len(), 3)
	}
	if pool.queue[account].Len() != 3 {
		t.Errorf("queued transaction mismatch: have %d, combt %d", pool.queue[account].Len(), 3)
	}
	if len(pool.all) != 6 {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), 6)
	}
	pool.lockedReset(nil, nil)
	if pool.pending[account].Len() != 3 {
		t.Errorf("pending transaction mismatch: have %d, combt %d", pool.pending[account].Len(), 3)
	...
	if len(pool.all) != 4 {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), 4)
	}
	...
	if len(pool.all) != 2 {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), 2)
	}
}
...
func TestTransactionPostponing(t *testing.T) {
	...
	if pool.pending[account].Len() != len(txns) {
		t.Errorf("pending transaction mismatch: have %d, combt %d", pool.pending[account].Len(), len(txns))
	}
	if len(pool.queue) != 0 {
		t.Errorf("queued transaction mismatch: have %d, combt %d", pool.queue[account].Len(), 0)
	}
	if len(pool.all) != len(txns) {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), len(txns))
	}
	pool.lockedReset(nil, nil)
	if pool.pending[account].Len() != len(txns) {
		t.Errorf("pending transaction mismatch: have %d, combt %d", pool.pending[account].Len(), len(txns))
	...
	if len(pool.all) != len(txns)/2 {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), len(txns)/2)
	}
}
...
func TestTransactionQueueAccountLimiting(t *testing.T) {
	...
	for i := uint64(1); i <= testTxPoolConfig.AccountQueue+5; i++ {
		...
		if len(pool.pending) != 0 {
			t.Errorf("tx %d: pending pool size mismatch: have %d, combt %d", i, len(pool.pending), 0)
		}
		if i <= testTxPoolConfig.AccountQueue {
			if pool.queue[account].Len() != int(i) {
				t.Errorf("tx %d: queue size mismatch: have %d, combt %d", i, pool.queue[account].Len(), i)
			}
		} else {
			if pool.queue[account].Len() != int(testTxPoolConfig.AccountQueue) {
				t.Errorf("tx %d: queue limit mismatch: have %d, combt %d", i, pool.queue[account].Len(), testTxPoolConfig.AccountQueue)
			}
		}
	}
	if len(pool.all) != int(testTxPoolConfig.AccountQueue) {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), testTxPoolConfig.AccountQueue)
	}
}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for addr, list := range pool.queue {
		if list.Len() > int(config.AccountQueue) {
			t.Errorf("addr %x: queued accounts overflown allocombce: %d > %d", addr, list.Len(), config.AccountQueue)
		}
		queued += list.Len()
	}
	if queued > int(config.GlobalQueue) {
		t.Fatalf("total transactions overflow allocombce: %d > %d", queued, config.GlobalQueue)
	}
	...
	if nolocals {
		...
		if len(pool.queue) != 1 {
			t.Errorf("multiple accounts in queue: have %v, combt %v", len(pool.queue), 1)
		}
		// Also ensure no local transactions are ever dropped, even if above global limits
		if queued := pool.queue[crypto.PubkeyToAddress(local.PublicKey)].Len(); uint64(queued) != 3*config.GlobalQueue {
			t.Fatalf("local account queued transaction count mismatch: have %v, combt %v", queued, 3*config.GlobalQueue)
		}
	}
}
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	pending, queued := pool.Stats()
	if pending != 0 {
		t.Fatalf("pending transactions mismatched: have %d, combt %d", pending, 0)
	}
	if queued != 2 {
		t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, 2)
	}
	...
	if nolocals {
		if queued != 0 {
			t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, 0)
		}
	} else {
		if queued != 1 {
			t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, 1)
		}
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
...
func TestTransactionPendingLimiting(t *testing.T) {
	...
	for i := uint64(0); i < testTxPoolConfig.AccountQueue+5; i++ {
		...
		if pool.pending[account].Len() != int(i)+1 {
			t.Errorf("tx %d: pending pool size mismatch: have %d, combt %d", i, pool.pending[account].Len(), i+1)
		}
		if len(pool.queue) != 0 {
			t.Errorf("tx %d: queue size mismatch: have %d, combt %d", i, pool.queue[account].Len(), 0)
		}
	}
	if len(pool.all) != int(testTxPoolConfig.AccountQueue+5) {
		t.Errorf("total transaction mismatch: have %d, combt %d", len(pool.all), testTxPoolConfig.AccountQueue+5)
	}
}
...
func TestTransactionPendingGlobalLimiting(t *testing.T) {
	...
	if pending > int(config.GlobalSlots) {
		t.Fatalf("total pending transactions overflow allocombce: %d > %d", pending, config.GlobalSlots)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
...
func TestTransactionPendingMinimumAllocombce(t *testing.T) {
	// Create the pool to test the limit enforcement with
	db, _ := ethdb.NewMemDatabase()
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(db))
	blockchain := &testBlockChain{statedb, big.NewInt(1000000), new(event.Feed)}
	...
	for addr, list := range pool.pending {
		if list.Len() != int(config.AccountSlots) {
			t.Errorf("addr %x: total pending transactions mismatch: have %d, combt %d", addr, list.Len(), config.AccountSlots)
		}
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
...
func TestTransactionPoolRepricing(t *testing.T) {
	...
	pending, queued := pool.Stats()
	if pending != 4 {
		t.Fatalf("pending transactions mismatched: have %d, combt %d", pending, 4)
	}
	if queued != 3 {
		t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, 3)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
	...
	pending, queued = pool.Stats()
	if pending != 2 {
		t.Fatalf("pending transactions mismatched: have %d, combt %d", pending, 2)
	}
	if queued != 3 {
		t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, 3)
	}
	...
	if err := pool.AddRemote(pricedTransaction(1, big.NewInt(100000), big.NewInt(1), keys[0])); err != ErrUnderpriced {
		t.Fatalf("adding underpriced pending transaction error mismatch: have %v, combt %v", err, ErrUnderpriced)
	}
	if err := pool.AddRemote(pricedTransaction(2, big.NewInt(100000), big.NewInt(1), keys[1])); err != ErrUnderpriced {
		t.Fatalf("adding underpriced queued transaction error mismatch: have %v, combt %v", err, ErrUnderpriced)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
	...
	if pending, _ = pool.Stats(); pending != 3 {
		t.Fatalf("pending transactions mismatched: have %d, combt %d", pending, 3)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	validate := func() {
		...
		if pending != expPending {
			t.Fatalf("pending transactions mismatched: have %d, combt %d", pending, expPending)
		}
		if queued != expQueued {
			t.Fatalf("queued transactions mismatched: have %d, combt %d", queued, expQueued)
		}
...
func TestTransactionReplacement(t *testing.T) {
	...
	if err := pool.AddRemote(pricedTransaction(0, big.NewInt(100001), big.NewInt(1), key)); err != ErrReplaceUnderpriced {
		t.Fatalf("original cheap pending transaction replacement error mismatch: have %v, combt %v", err, ErrReplaceUnderpriced)
	}
	...
	if err := pool.AddRemote(pricedTransaction(0, big.NewInt(100000), big.NewInt(threshold), key)); err != ErrReplaceUnderpriced {
		t.Fatalf("original proper pending transaction replacement error mismatch: have %v, combt %v", err, ErrReplaceUnderpriced)
	}
	...
	if err := pool.AddRemote(pricedTransaction(2, big.NewInt(100001), big.NewInt(1), key)); err != ErrReplaceUnderpriced {
		t.Fatalf("original queued transaction replacement error mismatch: have %v, combt %v", err, ErrReplaceUnderpriced)
	}
...
func TestStampVerifySuccess(t *testing.T) {
	...
	combStamp0dot1 := "1000000000000000"
	...
	dbMockRetVal, _ = new(big.Int).SetString(combStamp0dot1, 10)
