...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := GetHeader(db, header.Hash(), header.Number.Uint64()); entry == nil {
		...
		t.Fatalf("Retrieved header mismatch: have %v, combt %v", entry, header)
	}
	if entry := GetHeaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
		...
		if hash := common.BytesToHash(hasher.Sum(nil)); hash != header.Hash() {
			t.Fatalf("Retrieved RLP header mismatch: have %v, combt %v", entry, header)
		}
	}
...
func TestBodyStorage(t *testing.T) {
	...
	if entry := GetBody(db, hash, 0); entry == nil {
		...
		t.Fatalf("Retrieved body mismatch: have %v, combt %v", entry, body)
	}
	if entry := GetBodyRLP(db, hash, 0); entry == nil {
		...
		if calc := common.BytesToHash(hasher.Sum(nil)); calc != hash {
			t.Fatalf("Retrieved RLP body mismatch: have %v, combt %v", entry, body)
		}
	}
...
func TestBlockStorage(t *testing.T) {
	...
	if entry := GetBlock(db, block.Hash(), block.NumberU64()); entry == nil {
		...
		t.Fatalf("Retrieved block mismatch: have %v, combt %v", entry, block)
	}
	if entry := GetHeader(db, block.Hash(), block.NumberU64()); entry == nil {
		t.Fatalf("Stored header not found")
	} else if entry.Hash() != block.Header().Hash() {
		t.Fatalf("Retrieved header mismatch: have %v, combt %v", entry, block.Header())
	}
	if entry := GetBody(db, block.Hash(), block.NumberU64()); entry == nil {
		t.Fatalf("Stored body not found")
	} else if types.DeriveSha(types.Transactions(entry.Transactions)) != types.DeriveSha(block.Transactions()) || types.CalcUncleHash(entry.Uncles) != types.CalcUncleHash(block.Uncles()) {
		t.Fatalf("Retrieved body mismatch: have %v, combt %v", entry, block.Body())
	}
...
func TestTdStorage(t *testing.T) {
	...
	if entry := GetTd(db, hash, 0); entry == nil {
		...
		t.Fatalf("Retrieved TD mismatch: have %v, combt %v", entry, td)
	}
...
func TestCanonicalMappingStorage(t *testing.T) {
	...
	if entry := GetCanonicalHash(db, number); entry == (common.Hash{}) {
		...
		t.Fatalf("Retrieved canonical mapping mismatch: have %v, combt %v", entry, hash)
	}
	// Delete the TD and verify the execution
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := GetHeadHeaderHash(db); entry != blockHead.Hash() {
		t.Fatalf("Head header hash mismatch: have %v, combt %v", entry, blockHead.Hash())
	}
	if entry := GetHeadBlockHash(db); entry != blockFull.Hash() {
		t.Fatalf("Head block hash mismatch: have %v, combt %v", entry, blockFull.Hash())
	}
	if entry := GetHeadFastBlockHash(db); entry != blockFast.Hash() {
		t.Fatalf("Fast head block hash mismatch: have %v, combt %v", entry, blockFast.Hash())
	}
}
	...
	for i, tx := range txs {
		if txn, hash, number, index := GetTransaction(db, tx.Hash()); txn == nil {
			...
			if hash != block.Hash() || number != block.NumberU64() || index != uint64(i) {
				t.Fatalf("tx #%d [%x]: positional metadata mismatch: have %x/%d/%d, combt %x/%v/%v", i, tx.Hash(), hash, number, index, block.Hash(), block.NumberU64(), i)
			}
			if tx.String() != txn.String() {
				t.Fatalf("tx #%d [%x]: transaction mismatch: have %v, combt %v", i, tx.Hash(), txn, tx)
			}
		}
	}
...
func TestBlockReceiptStorage(t *testing.T) {
	...
	if rs := GetBlockReceipts(db, hash, 0); len(rs) == 0 {
		...
		for i := 0; i < len(receipts); i++ {
			...
			rlpcombt, _ := rlp.EncodeToBytes(receipts[i])

			if !bytes.Equal(rlpHave, rlpcombt) {
				t.Fatalf("receipt #%d: receipt mismatch: have %v, combt %v", i, rs[i], receipts[i])
			}
		}
	}
