...
func TestHeaderVerification(t *testing.T) {
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			case result := <-results:
				if (result == nil) != valid {
					t.Errorf("test %d.%d: validity mismatch: have %v, combt %v", i, j, result, valid)
				}
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		for j := 0; j < len(blocks); j++ {
			combt := valid || (j < len(blocks)-2) // We chose the last-but-one nonce in the chain to fail
			if (checks[j] == nil) != combt {
				t.Errorf("test %d.%d: validity mismatch: have %v, combt %v", i, j, checks[j], combt)
			}
			if !combt {
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	if verified > 2*threads {
		t.Errorf("verification count too large: have %d, combt below %d", verified, 2*threads)
	}
}
