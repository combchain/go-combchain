...
func testScheduler(t *testing.T, clients int, fetchers int, requests int) {
	...
	for i := 0; i < clients; i++ {
		go func() {
			...
			for j := 0; j < requests; j++ {
				bits := <-out
				if combt := new(big.Int).SetUint64(uint64(j)).Bytes(); !bytes.Equal(bits, combt) {
					t.Errorf("vector %d: delivered content mismatch: have %x, combt %x", j, bits, combt)
				}
			}
		}()
	}
	...
	if have := atomic.LoadUint32(&delivered); int(have) != requests {
		t.Errorf("request count mismatch: have %v, combt %v", have, requests)
	}
}
