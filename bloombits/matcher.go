...
// of pending requests, which are assigned to retrievers combting to fulfil them.
func (m *Matcher) distributor(dist chan *request, session *MatcherSession) {
	...
	for {
		...
		case fetcher := <-m.retrievals:
			...
			task := <-fetcher
			if combt := len(task.Sections); combt >= len(requests[task.Bit]) {
				task.Sections = requests[task.Bit]
				delete(requests, task.Bit)
			} else {
				task.Sections = append(task.Sections[:0], requests[task.Bit][:combt]...)
				requests[task.Bit] = append(requests[task.Bit][:0], requests[task.Bit][combt:]...)
			}
			fetcher <- task
