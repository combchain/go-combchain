...
func TestMatcherWildcards(t *testing.T) {
	...
	if len(matcher.filters) != 3 {
		t.Fatalf("filter system size mismatch: have %d, combt %d", len(matcher.filters), 3)
	}
	if len(matcher.filters[0]) != 2 {
		t.Fatalf("address clause size mismatch: have %d, combt %d", len(matcher.filters[0]), 2)
	}
	if len(matcher.filters[1]) != 2 {
		t.Fatalf("combo topic clause size mismatch: have %d, combt %d", len(matcher.filters[1]), 2)
	}
	if len(matcher.filters[2]) != 1 {
		t.Fatalf("singletone topic clause size mismatch: have %d, combt %d", len(matcher.filters[2]), 1)
	}
}
...
func testMatcher(t *testing.T, filter [][]bloomIndexes, blocks uint64, intermittent bool, retrievals uint32, maxReqCount int) uint32 {
	...
	if retrievals != 0 && requested != retrievals {
		t.Errorf("filter = %v  blocks = %v  intermittent = %v: request count mismatch, have #%v, combt #%v", filter, blocks, intermittent, requested, retrievals)
	}
	return requested
}
