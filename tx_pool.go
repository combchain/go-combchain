...
var (
	...
	// maximum allocombce of the current block.
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	// If the pending limit is overflown, start equalizing allocombces
	pending := uint64(0)
	for _, list := range pool.pending {
	...
	if pending > pool.config.GlobalSlots {
		...
		// If still above threshold, reduce to limit or min allocombce
