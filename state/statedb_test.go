...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		checkeq := func(op string, a, b interface{}) bool {
			if err == nil && !reflect.DeepEqual(a, b) {
				err = fmt.Errorf("got %s(%s) == %v, combt %v", op, addr.Hex(), a, b)
				return false
			}
			return true
		}
	...
	if state.GetRefund().Cmp(checkstate.GetRefund()) != 0 {
		return fmt.Errorf("got GetRefund() == %d, combt GetRefund() == %d",
			state.GetRefund(), checkstate.GetRefund())
	}
	if !reflect.DeepEqual(state.GetLogs(common.Hash{}), checkstate.GetLogs(common.Hash{})) {
		return fmt.Errorf("got GetLogs(common.Hash{}) == %v, combt GetLogs(common.Hash{}) == %v",
