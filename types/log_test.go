...
var unmarshalLogTests = map[string]struct {
	...
	"missing data": {
		...
		combtError: fmt.Errorf("missing required field 'data' for Log"),
	},
}
...
func TestUnmarshalLog(t *testing.T) {
	...
	for name, test := range unmarshalLogTests {
		...
		checkError(t, name, err, test.combtError)
		if test.combtError == nil && err == nil {
			if !reflect.DeepEqual(log, test.combt) {
				t.Errorf("test %q:\nGOT %scombT %s", name, dumper.Sdump(log), dumper.Sdump(test.combt))
			}
		}
	}
}

func checkError(t *testing.T, testname string, got, combt error) bool {
	if got == nil {
		if combt != nil {
			t.Errorf("test %q: got no error, combt %q", testname, combt)
			return false
		}
		return true
	}
	...
	} else if got.Error() != combt.Error() {
		t.Errorf("test %q: got error %q, combt %q", testname, got, combt)
	}
	return false
}
