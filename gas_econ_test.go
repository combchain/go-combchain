...
var (
	combCoinSCAddr = common.BytesToAddress([]byte{100})
...
func TestGasOrdinaryCoinTransfer(t *testing.T) {
	var (
		...
		// value of comb coin to transfer
...
func TestGasCoinMint(t *testing.T) {
	...
	chain, _ := chainEnv.GenerateChainMulti(genesis, 1, func(i int, gen *BlockGen) {
		gen.SetCoinbase(coinbase)
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(sender), combCoinSCAddr, transferValue, gl, gp, mintCoinData), signer, sk)
		gasUsed = gen.AddTxAndCalcGasUsed(tx)
	})
...
func TestGasCoinRefund(t *testing.T) {
	...
	chain, _ := chainEnv.GenerateChainMulti(genesis, 1, func(i int, gen *BlockGen) {
		...
		// add transactions
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(sender), combCoinSCAddr, transferValue, gl, gp, mintCoinData), signer, sk)
		txC1, _ := types.SignTx(types.NewTransaction(gen.TxNonce(contributor1), combCoinSCAddr, transferValue, gl, gp, mintCoinDataContributor1), signerC1, skContributor1)
		txC2, _ := types.SignTx(types.NewTransaction(gen.TxNonce(contributor2), combCoinSCAddr, transferValue, gl, gp, mintCoinDataContributor2), signerC2, skContributor2)
...
func genOTAStr(pk, pk1 *ecdsa.PublicKey) (string, error) {
	...
	OTAcombFormatRaw, err := keystore.WaddrFromUncompressedRawBytes(OTARaw)
	if err != nil {
		return "", errOTAGen
	}

	return hexutil.Encode(OTAcombFormatRaw[:]), nil
}

// generate data for comb coin mint transaction
...
func getOTABalance(db *state.StateDB, ota string) *big.Int {
	otaAX, _ := vm.GetAXFromcombAddr(common.FromHex(ota))
