...
func (c *ecrecover) Run(input []byte, contract *Contract, evm *EVM) ([]byte, error) {
	...
	// but for ecrecover we combt (r, s, v)
...
var (
	...
	errCoinValue = errors.New("combcoin value is not support")

	ErrMismatchedValue = errors.New("mismatched combcoin value")
...
const (
	combcoin10  = "10000000000000000000"  //10
	combcoin20  = "20000000000000000000"  //20
	combcoin50  = "50000000000000000000"  //50
	combcoin100 = "100000000000000000000" //100

	combcoin200   = "200000000000000000000"   //200
	combcoin500   = "500000000000000000000"   //500
	combcoin1000  = "1000000000000000000000"  //1000
	combcoin5000  = "5000000000000000000000"  //5000
	combcoin50000 = "50000000000000000000000" //50000

	combStampdot001 = "1000000000000000" //0.001
	combStampdot002 = "2000000000000000" //0.002
	combStampdot005 = "5000000000000000" //0.005

	combStampdot003 = "3000000000000000" //0.003
	combStampdot006 = "6000000000000000" //0.006
	combStampdot009 = "9000000000000000" //0.009

	combStampdot03 = "30000000000000000"  //0.03
	combStampdot06 = "60000000000000000"  //0.06
	combStampdot09 = "90000000000000000"  //0.09
	combStampdot2  = "200000000000000000" //0.2
	combStampdot3  = "300000000000000000" //0.3
	combStampdot5  = "500000000000000000" //0.5
...
func init() {
	...
	svaldot001, _ := new(big.Int).SetString(combStampdot001, 10)
	StampValueSet[svaldot001.Text(16)] = combStampdot001

	svaldot002, _ := new(big.Int).SetString(combStampdot002, 10)
	StampValueSet[svaldot002.Text(16)] = combStampdot002

	svaldot005, _ := new(big.Int).SetString(combStampdot005, 10)
	StampValueSet[svaldot005.Text(16)] = combStampdot005

	svaldot003, _ := new(big.Int).SetString(combStampdot003, 10)
	StampValueSet[svaldot003.Text(16)] = combStampdot003

	svaldot006, _ := new(big.Int).SetString(combStampdot006, 10)
	StampValueSet[svaldot006.Text(16)] = combStampdot006

	svaldot009, _ := new(big.Int).SetString(combStampdot009, 10)
	StampValueSet[svaldot009.Text(16)] = combStampdot009

	svaldot03, _ := new(big.Int).SetString(combStampdot03, 10)
	StampValueSet[svaldot03.Text(16)] = combStampdot03

	svaldot06, _ := new(big.Int).SetString(combStampdot06, 10)
	StampValueSet[svaldot06.Text(16)] = combStampdot06

	svaldot09, _ := new(big.Int).SetString(combStampdot09, 10)
	StampValueSet[svaldot09.Text(16)] = combStampdot09

	svaldot2, _ := new(big.Int).SetString(combStampdot2, 10)
	StampValueSet[svaldot2.Text(16)] = combStampdot2

	svaldot3, _ := new(big.Int).SetString(combStampdot3, 10)
	StampValueSet[svaldot3.Text(16)] = combStampdot3

	svaldot5, _ := new(big.Int).SetString(combStampdot5, 10)
	StampValueSet[svaldot5.Text(16)] = combStampdot5

	cval10, _ := new(big.Int).SetString(combcoin10, 10)
	combCoinValueSet[cval10.Text(16)] = combcoin10

	cval20, _ := new(big.Int).SetString(combcoin20, 10)
	combCoinValueSet[cval20.Text(16)] = combcoin20

	cval50, _ := new(big.Int).SetString(combcoin50, 10)
	combCoinValueSet[cval50.Text(16)] = combcoin50

	cval100, _ := new(big.Int).SetString(combcoin100, 10)
	combCoinValueSet[cval100.Text(16)] = combcoin100

	cval200, _ := new(big.Int).SetString(combcoin200, 10)
	combCoinValueSet[cval200.Text(16)] = combcoin200

	cval500, _ := new(big.Int).SetString(combcoin500, 10)
	combCoinValueSet[cval500.Text(16)] = combcoin500

	cval1000, _ := new(big.Int).SetString(combcoin1000, 10)
	combCoinValueSet[cval1000.Text(16)] = combcoin1000

	cval5000, _ := new(big.Int).SetString(combcoin5000, 10)
	combCoinValueSet[cval5000.Text(16)] = combcoin5000

	cval50000, _ := new(big.Int).SetString(combcoin50000, 10)
	combCoinValueSet[cval50000.Text(16)] = combcoin50000

}

type combchainStampSC struct{}

func (c *combchainStampSC) RequiredGas(input []byte) uint64 {
	// ota balance store gas + ota combaddr store gas
	return params.SstoreSetGas * 2
}

func (c *combchainStampSC) Run(in []byte, contract *Contract, env *EVM) ([]byte, error) {
	if len(in) < 4 {
...
func (c *combchainStampSC) ValidTx(stateDB StateDB, signer types.Signer, tx *types.Transaction) error {
...
func (c *combchainStampSC) ValidBuyStampReq(stateDB StateDB, payload []byte, value *big.Int) (otaAddr []byte, err error) {
	...
	combAddr, err := hexutil.Decode(StampInput.OtaAddr)
	if err != nil {
		return nil, err
	}

	ax, err := GetAXFromcombAddr(combAddr)
...
func (c *combchainStampSC) buyStamp(in []byte, contract *Contract, evm *EVM) ([]byte, error) {
	combAddr, err := c.ValidBuyStampReq(evm.StateDB, in, contract.value)
	if err != nil {
		return nil, err
	}

	add, err := AddOTAIfNotExist(evm.StateDB, contract.value, combAddr)
	if err != nil || !add {
		return nil, errBuyStamp
	}
...
type combCoinSC struct {
}

func (c *combCoinSC) RequiredGas(input []byte) uint64 {
	if len(input) < 4 {
		return 0
	}
...
func (c *combCoinSC) Run(in []byte, contract *Contract, evm *EVM) ([]byte, error) {
	if len(in) < 4 {
		return nil, errParameters
	}
...
func (c *combCoinSC) ValidTx(stateDB StateDB, signer types.Signer, tx *types.Transaction) error {
	if stateDB == nil || signer == nil || tx == nil {
		return errParameters
	}
...
func (c *combCoinSC) ValidBuyCoinReq(stateDB StateDB, payload []byte, txValue *big.Int) (otaAddr []byte, err error) {
	...
	_, ok := combCoinValueSet[outStruct.Value.Text(16)]
	if !ok {
		return nil, errCoinValue
	}

	combAddr, err := hexutil.Decode(outStruct.OtaAddr)
	if err != nil {
		return nil, err
	}
...
func (c *combCoinSC) buyCoin(in []byte, contract *Contract, evm *EVM) ([]byte, error) {
...
func (c *combCoinSC) ValidRefundReq(stateDB StateDB, payload []byte, from []byte) (image []byte, value *big.Int, err error) {
...
func (c *combCoinSC) refund(all []byte, contract *Contract, evm *EVM) ([]byte, error) {
...
func GetSupportcombCoinOTABalances() []*big.Int {
	cval10, _ := new(big.Int).SetString(combcoin10, 10)
	cval20, _ := new(big.Int).SetString(combcoin20, 10)
	cval50, _ := new(big.Int).SetString(combcoin50, 10)
	cval100, _ := new(big.Int).SetString(combcoin100, 10)
