...
//
// 1 combcoin --> (bigint)1000000000000000000 --> "0x0000000000000000000001000000000000000000"
//
...
// GetAXFromcombAddr retrieve ota AX from ota combAddr
func GetAXFromcombAddr(otacombAddr []byte) ([]byte, error) {
	if len(otacombAddr) != common.WAddressLength {
		return nil, ErrInvalidOTAAddr
	}

	return otacombAddr[1 : 1+common.HashLength], nil
}

// IsAXPointTocombAddr check whether AX point to otacombAddr or not
func IsAXPointTocombAddr(AX []byte, otacombAddr []byte) bool {
	findAX, err := GetAXFromcombAddr(otacombAddr)
	if err != nil {
		return false
	}
...
// setOTA storage ota info, include balance and combAddr. Overwrite if ota exist already.
func setOTA(statedb StateDB, balance *big.Int, otacombAddr []byte) error {
	if statedb == nil || balance == nil {
		return ErrUnknown
	}
	if len(otacombAddr) != common.WAddressLength {
	...
	otaAX, _ := GetAXFromcombAddr(otacombAddr)
	...
	mptAddr := OTABalance2ContractAddr(balance)
	statedb.SetStateByteArray(mptAddr, common.BytesToHash(otaAX), otacombAddr)
	return SetOtaBalanceToAX(statedb, otaAX, balance)
}

// AddOTAIfNotExist storage ota info if doesn't exist already.
func AddOTAIfNotExist(statedb StateDB, balance *big.Int, otacombAddr []byte) (bool, error) {
	if statedb == nil || balance == nil {
	...
	err = setOTA(statedb, balance, otacombAddr)
	if err != nil {
		return false, err
	}
...
// GetOTAInfoFromAX retrieve ota info, include balance and combAddr
func GetOTAInfoFromAX(statedb StateDB, otaAX []byte) (otacombAddr []byte, balance *big.Int, err error) {
	if statedb == nil {
...
func (env *GetOTASetEnv) OTAInSet(ota []byte) bool {
	for _, exist := range env.otacombAddrSet {
...
func (env *GetOTASetEnv) RandomSelOTA(value []byte) bool {
	...
	if env.loopTimes%env.rnd == 0 {
		env.otacombAddrSet = append(env.otacombAddrSet, value)
		env.getNum++
		env.UpdateRnd()
		return true
	} else {
...
func doOTAStorageTravelCallBack(env *GetOTASetEnv, value []byte) (bool, error) {
	...
	if IsAXPointTocombAddr(env.otaAX, value) {
		return true, nil
	}
...
//		5: If find invalid ota combaddr, return error;
...
//
func GetOTASet(statedb StateDB, otaAX []byte, setNum int) (otacombAddrs [][]byte, balance *big.Int, err error) {
	if statedb == nil {
		return nil, nil, ErrUnknown
	}
	if len(otaAX) != common.HashLength {
	...
	env.otacombAddrSet = make([][]byte, 0, setNum)
	env.UpdateRnd()
	...
	for {
		...
		if env.IsSetFull() {
			return env.otacombAddrSet, balance, nil
		} else if err != nil {
			return nil, nil, err
