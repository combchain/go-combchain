...
//		)
//
//		err := setOTA(statedb, balanceSet, otacombAddr)
//		if err != nil {
//			t.Error("set ota balance fail. err:", err.Error())
//		}
//
...
//			if !bytes.Equal(otaGet, otacombAddr) {
//				t.Error("ota addr in set is wrong! expect:", common.ToHex(otacombAddr), ", actual:", common.ToHex(otaGet))
//			}
//		}
//
//		if balanceGet == nil {
...
//		)
//
//		for _, otacombAddr := range otaMixSetAddrs {
//			otaMixSetAddrBytes = append(otaMixSetAddrBytes, common.FromHex(otacombAddr))
//		}
//
//		err := setOTA(statedb, balanceSet, otacombAddr)
//		if err != nil {
...
//		for _, otaGet := range otaSet {
//			AXGet, _ := GetAXFromcombAddr(otaGet)
//			copy(otaGetAX[:], AXGet)
...
func TestGetOTASet(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if bytes.Equal(otaSet[0], otacombAddr) {
			t.Error("otaSet value wrong!, contain unexpected ota")
		}
